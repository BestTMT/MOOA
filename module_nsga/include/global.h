...
individual_refpoints;  //个体的引用点

typedef struct
{
    individual_refpoints *ind;
}
population_refpoints; //总体引用点
...
typedef struct
{
    ...
    double *xreal; //real number  xreal跟real number的个数是有关的；
    double *xbin;  //binary number
    int **gene; // related with xbin variable
    double *obj; // store final result
...
typedef struct
{
    individual *ind;  //this is not multyply, this is struct object.
}
population;

typedef struct lists
{
    int index; //seems to store the individual's index in the population.
    struct lists *parent;
    struct lists *child;
}
list;
