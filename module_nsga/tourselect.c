...
void selection (population *old_pop, population *new_pop)
{
    ...
    for (i=0; i<popsize; i++)
    {
        a1[i] = a2[i] = i; // id's sequence
    }
    for (i=0; i<popsize; i++) // the target is till the random choice.
    {
    ...
    {
        parent1 = tournament (&old_pop->ind[a1[i]], &old_pop->ind[a1[i+1]]); //only based on feasiable selection
...
individual* tournament (individual *ind1, individual *ind2)
{
    ...
    if ((randomperc()) <= 0.5) //if same dominance, then return either is okay.
    {
        return(ind1);
    }
    else
