...
void fill_nondominated_sort(population *selection_pop, population *mixed_pop,
  ...
  list *elite;
  list *temp1, *temp2;
  pool = (list *)malloc(sizeof(list)); // a simple list implemented by C
  ...
  /*bubble_sorting_infeasible_population_index() function, stores infeasible
  solutions index and sorts them from higher to lower constrain violation*/
  bubble_sorting_infeasible_population_index(mixed_pop);  //seems only do sorting for infeasible solution.

  /*printf("number_is_infeasible %d, number_is_feasible %d, 2*popsize
  %d\n",number_is_infeasible,number_is_feasible,2*popsize); printf("\nThere are
  4 cases for fill nondominated sort\n");*/
  /*printf("******************************************************************************************************************\n");
  ...
  printf("Case 1 and 2 do not associate reference points. Cases 3 and 4 do not
  consider infeasible solutions at all\n");
  printf("*****************************************************************************************************************\n\n");*/

  if (number_is_infeasible == 2 * popsize ||   //seems this case is the extreme case.
      (number_is_feasible < popsize && number_is_feasible > 0)) {
    if (number_is_infeasible == 2 * popsize) {
      /*printf("\nCase 1. All solutions are infeasible. Solutions sorted by
      minimum constrain violation values.\n\n"); printf("number_is_feasible %d,
      number_is_infeasible %d, popsize
      %d\n",number_is_feasible,number_is_infeasible,popsize);*/
      k = 0;
      for (i = 0; i < popsize; i++) {
        copy_ind(&mixed_pop->ind[infeasible_population_sorted_list_index[i]],
                 &new_pop->ind[i]);
      }

      /*printf("Visualization all infeasible solutions of the new_pop sorted by
      ...
      if (number_is_feasible < popsize && number_is_feasible > 0) {
        /*printf("\nCase 2: Number_is_feasible < popsize, number_is_feasible
        %d\n\n",number_is_feasible); printf("number_is_feasible %d,
        number_is_infeasible %d, popsize
        %d\n",number_is_feasible,number_is_infeasible,popsize);*/
        for (i = 0; i < number_is_feasible; i++) {
          copy_ind(&mixed_pop->ind[feasible_population_sorted_list_index[i]],
                   &new_pop->ind[i]);
        }

        for (i = number_is_feasible, j = 0; i < popsize; i++, j++) {
          copy_ind(&mixed_pop->ind[infeasible_population_sorted_list_index[j]],
                   &new_pop->ind[i]);
        }

        /*printf("Visualization feasible solutions of the selection_pop
         * number_is_feasible<=popsize\n");*/
        /*for (k=0;k<number_is_feasible; k++)
        {
                display_pop_ind_obj(&(new_pop->ind[k]),k);
        }*/
        /*printf("Visualization infeasible solutions of the selection_pop
         * number_is_feasible<=popsize\n");*/
        ...
        /*printf("Visualization of the new_pop infeasible/feasible
         * solutions\n");*/
        /*for (k=0;k<popsize; k++)
        {
                display_pop_ind_obj(&(new_pop->ind[k]),k);
    ...
    if (number_is_feasible >= popsize) {
      if (number_is_feasible == 2 * popsize) {
        ...
        printf("number_is_feasible %d, number_is_infeasible %d, popsize
        %d\n",number_is_feasible,number_is_infeasible,popsize);*/
        ...
        if (number_is_feasible >= popsize && number_is_feasible < 2 * popsize) {
          ...
          for (i = 0; i < number_is_feasible; i++) {
            ...
            temp1 = temp1->child;
          }// seems do the list insert conduction.
        }
      }
      i = 0;
      do {
        ...
        temp2 = elite->child;
        j = i;
        /*printf("archieve_size %d, front_size
        ...
        if ((archieve_size + front_size) <= popsize) {
          do {
            ...
            if (archieve_size == popsize) {
              printf(
                  "Lucky generation, archieve_size==popsize, Nothing to do. "
          ...
          /*printf("1. visualization of the selection_pop without the last front
           * in the fillnds.c file\n");*/
          /*for (k=0;k<archieve_size; k++)
          {
              display_pop_ind_obj(&(selection_pop->ind[k]),k);
          }*/
          associated_reference_points_fill(selection_pop, mixed_pop, new_pop,
                                           front_size, archieve_size, elite,
                                           generation);
          /*printf("archieve_size is %d, front_size is %d, i or count is %d, j
...
/* Routine to fill a population with individuals associated with reference
 * points (Das and Dennis's) and (Deb and Jain)*/
void associated_reference_points_fill(population *selection_pop,
                                      population *mixed_pop,
                                      population *new_pop, int front_size,
                                      int archieve_size, list *elite,
                                      int generation) {
  int j, k, l;
  ...
  archieve_and_front_sizes =
      variables_initialization(selection_pop, mixed_pop, new_pop, front_size,
                               archieve_size, elite); /*ok*/
  if (archieve_size == popsize) {
  ...
  pop_size = (archieve_size > 0 && archieve_size < popsize)
                 ? archieve_size
                 : archieve_and_front_sizes;
  /*printf("archieve_size is %d, front_size is
   * %d\n",archieve_size,front_size);*/
  ...
  /*for (j=0; j<nobj; j++)
  {
      printf("4. scale_obj_min[%d][index=%d]
  %e\n",j,scale_obj_min_ref,scale_obj_min[j]);
  }*/
  /*visualization of max point*/
  /*for (j=0; j<nobj; j++)
  {
      printf("4. scale_obj_max[%d][index=%d]
  ...
  printf("5. visualization of the whole selection_pop minus zmin in the
  ...
  find_extreme_points(
      selection_pop,
      pop_size); /*only consider the individuals in the first front*/
  /*normalize solutions*/
  ...
  printf("7. visualization of the whole selection_pop normalized in the
  fillnds.c file\n"); for (l=0; l<archieve_and_front_sizes; l++)
  {
  ...
  for (l = 0; l < archieve_and_front_sizes; l++) {
    associate(
        &selection_pop->ind[l], l, archieve_size, 0,
        factorial + factorial_inside + last_gen_adaptive_refpoints_number);
  }
  /*printf("Printing rho after asociation refpoints ((factorial
   * %d-adaptive_ref_points_inserted %d=%d)+factorial_inside %d
  ...
  /*printf("After association: temp_rho_St_total %d, temp_rho_Fl_total
  ...
  member_number = niching(
      selection_pop, new_pop, front_size, archieve_size, 0,
      factorial + factorial_inside + last_gen_adaptive_refpoints_number);
  ...
  /*printf("After niching: temp_rho_St_total %d, temp_rho_Fl_total
  %d\n",temp_rho_St_total,temp_rho_Fl_total); printf("member_number %d,
  archieve_size %d,total %d, popsize
  ...
  if (adaptive_nsga == 1 || adaptive_nsga == 2) {
    ...
    last_gen_adaptive_refpoints_number = delete_adaptive_refpoints(
        archieve_size, front_size, selection_pop, new_pop, generation);
    /*display_refpoints ();*/
    ...
    /*for (l=0; l<archieve_and_front_sizes; l++)
    {
            associate(&(selection_pop->ind[l]),&(new_pop->ind[l]),l,
    archieve_size);
    }*/
...
int bubble_sorting_infeasible_population_index(population *poputation_sorted) {
  ...
  double CV[2 * popsize]; //constraint violation value

  number_is_feasible = 0;
  number_is_infeasible = 0;

  for (i = 0; i < 2 * popsize; i++) {
    /*Checking feasibility of mixed solutions*/  // deal with infeasible cases
    if (poputation_sorted->ind[i].constr_violation +
            poputation_sorted->ind[i].equality_constr_violation <
        0) {
      infeasible_population_sorted_list_index[number_is_infeasible] = i;
      CV[number_is_infeasible] =
          poputation_sorted->ind[i].constr_violation +
          poputation_sorted->ind[i].equality_constr_violation;
      poputation_sorted->ind[i].is_feasible = 0;
      number_is_infeasible++;
      /*printf("%d,is infeasible
       * %d\n",i,poputation_sorted->ind[i].is_feasible);*/
    } else { // deal with feasible cases
      feasible_population_sorted_list_index[number_is_feasible] = i;
      poputation_sorted->ind[i].is_feasible = 1;
      number_is_feasible++;
      /*printf("%d,is feasible %d, number_is_feasible
       * %d\n",i,poputation_sorted->ind[i].is_feasible,number_is_feasible);*/
    }
  }
  if (number_is_feasible == 2 * popsize) return number_is_feasible;
  for (i = 0; i < number_is_infeasible; i++) {
    for (j = 0; j < number_is_infeasible - i - 1; j++) {
      if (CV[j] < CV[j + 1]) {
        ...
        CV[j] = CV[j + 1];
        infeasible_population_sorted_list_index[j] =
            infeasible_population_sorted_list_index[j + 1];
        CV[j + 1] = temp;
