...
int generate_DTLZ1(int nobj_for, double step) {
  int i, count;
  count = recursive_for_DTLZ1(nobj_for, step, 0,
                              0);  // count's target is to get 91 population
  return count;
}
int recursive_for_DTLZ1(int nobj_for, double step, int count, int i) {
	//this place seems a demo, five parameters,  3 targets.
  int j;

  if (count == factorial) return 1;
  if (count > factorial) {
    printf(
        "Error, count > factorial %d, then exit!, check recursive_for "
        "function\n",
        factorial);
    exit(-1);
  }
  for (j = 0; j < numberpointperdim - i && count < factorial; j++) {
    ...
    index_p[nobj - 1 - nobj_for_temp] = j;
    ...
    if (nobj_for_temp == 1) {
      ...
      for (m = n_var - k1; m < n_var; m++) {
        g += (index_p[m] - 0.5) * (index_p[m] - 0.5) -
             cos(20.0 * PI *
                 (index_p[m] - 0.5));  // index_p as the index parameter.
      }
      g = 100 * (k1 + g);
      for (k = 0; k < nobj - 1; k++) {
        DTLZ[k][count] = 0.5 - 0.5 * step * index_p[k];
        temp += index_p[k];
      }
      DTLZ[k][count] =
          0.5 -
          0.5 *
              (1 - (step * temp));  // DTLZ's content. As the last still has one
                                    // line, so we need to populate the data.
      count++;
    }
...
int recursive_for(int nobj_for, double step, int count, int i) {
  ...
  for (j = 0; j < numberpointperdim - i && count < factorial; j++) {
    ...
    if (nobj_for_temp == 1) { // nobj_for_temp special for the last parameter
      int temp = 0;
      int k;
      for (k = 0; k < nobj - 1; k++) { // if nobj = 3, this segment is the first two parameter, 
        ref_points[k][count + factorial_inside] = step * index_p[k];
        temp += index_p[k];
      }
      ref_points[k][count + factorial_inside] = 1 - step * (temp); // if nobj = 3, this segment is the last parameter, 
      count++;
    }
    if (nobj_for_temp > 1 && count < factorial) {
      nobj_for_temp--;
      count = recursive_for(nobj_for_temp, step, count, i + j); // utilzie i + j to take control the innel I
    }
  }
...
int recursive_for_inside(int nobj_for, double step, int count, int i) {
  int j;
  if (count == factorial_inside) return 1;
  if (count > factorial_inside) {
    printf(
        "error, count > factorial_inside %d, then exit!, check "
        "recursive_for function\n",
        factorial_inside);
    exit(-1);
  }
  for (j = 0; j < numberpointperdim_inside - i && count < factorial_inside;
       j++) {
    int nobj_for_temp = nobj_for;
    index_p[nobj - 1 - nobj_for_temp] = j;
    ...
    if (nobj_for_temp == 1) {
      ...
      for (k = 0; k < nobj - 1; k++) {
        ref_points[k][count] =
            (1 - step) * step * index_p[k] + (double)step / nobj;
        temp += index_p[k];
      }
      ref_points[k][count] =
          (1 - step) * (1 - step * (temp)) + (double)step / nobj;
      count++;
    }
...
int recursive_for_adaptive(int nobj_for, double step, int count, int i) {
  ...
  if (count == factorial_adaptive) return 1;
  if (count > factorial_adaptive) {
    printf(
        "error, count > factorial_adaptive = %d, then exit!, check "
        "recursive_for function\n",
        factorial_adaptive);
    exit(-1);
  }
    ...
    if (nobj_for_temp == 1) {
      ...
      for (k = 0; k < nobj - 1; k++) {
        minimum_amount_ref_points[k][count] =
            (1 / (double)(numberpointperdim - 1)) * step * index_p[k];
        temp += index_p[k];
      }

      minimum_amount_ref_points[k][count] =
          (1 / (double)(numberpointperdim - 1)) * (1 - step * (temp));
      count++;
    }
    if (nobj_for_temp > 1 && count < factorial_adaptive) {
      nobj_for_temp--;
      count = recursive_for_adaptive(nobj_for_temp, step, count, i + j); //minimum_amount_ref_points populate data into this fields.
    }
  }
...
void refpoints_normalized() {
  ...
  for (i = 0; i < factorial + factorial_inside; i++) {
    for (j = 0; j < nobj; j++) {
      ref_points_normalized[j][i] = (ref_points[j][i] - min_ref_points[j]) /
                                    (max_ref_points[j] - min_ref_points[j]);
    }
  }
  return;
}
...
int create_adaptive_refpoints() {
  /*This function take information of the updated niche count (Next size
   * population size(P(t+1))=N )*/
  int i, j, k, l, m, n;
  int crowded_rho = 1;
  int temp = 0;
  int temp_j = 0;
  ...
  /*find_useless_refpoints_index finds index for useless_refpoints and
   * initialize usefull_refpoint_number*/
  find_useless_usefull_refpoint_index();

  /*printf("\nFinding the number of useless and usefull reference
   * points\n_______________________________________________\n");*/

  for (i = 0; i < usefull_refpoint_number; i++) {
    for (j = 0; j < factorial_adaptive; j++) {
      /*That is true because refpoints are sorted from the highest to the lowest
      value. So, it requires to do only usefull_refpoints_number times*/
      if (rho_St[sort_all_refpoint_index[i]] > crowded_rho) {
        if (adaptive_nsga == 1) {
          /*printf("Adaptive refpoints:\n");*/
          for (k = 0; k < nobj; k++) {
            /*Generation and Translation of new reference points to the crowded
             * reference point*/
            adaptive_refpoints[k][temp_j] =
                minimum_amount_ref_points[k][j] +
                ref_points[k][sort_all_refpoint_index[i]] -
                (1 / (double)(numberpointperdim - 1)) / nobj;
          }
          /*Verification:reference points adaptive neither outside the first
           * quadrant nor repeated*/
          for (l = 0; l < factorial + factorial_inside; l++) {
            temp_nobj = 0;
            temp_outside_first_quadrant = 0;
            for (k = 0; k < nobj; k++) {
              if (fabs(adaptive_refpoints[k][temp_j] - ref_points[k][l]) <
                  1e-15) {
                temp_nobj++;
              }
          ...
          /*Be sure there is not repeated the new adaptive reference point,
           * before add it*/
          for (m = 0; m < temp_j; m++) {
            temp_nobj_adaptive = 0;
            temp_adaptive_refpoint_repeated = 0;
            for (k = 0; k < nobj; k++) {
              if (fabs(adaptive_refpoints[k][temp_j] -
                       adaptive_refpoints[k][m]) < 1e-15)
                temp_nobj_adaptive++;
            }
          ...
          if (!(temp_nobj == nobj || temp_outside_first_quadrant ||
                temp_adaptive_refpoint_repeated)) {
            temp_j++;
          }
        }
        if (adaptive_nsga == 2) {
          ...
          for (l = 0; l < nobj; l++) {
            for (k = 0; k < nobj; k++) {
              translation[k][l] =
                  0.5 * minimum_amount_ref_points[k][l] -
                  (1 / (double)(numberpointperdim - 1)) / (2 * nobj);
            }
          }

          for (k = 0; k < nobj; k++) {
            for (l = 0; l < nobj; l++) {
              adaptive_refpoints[l][temp_j] =
                  translation[l][k] - translation[l][j] +
                  ref_points[l][sort_all_refpoint_index[i]];
            }
            /*Verification:reference points adaptive neither outside the first
             * quadrant nor repeated*/
            /*It either repeated of outside the first quadrant, assign the
             * vector 0*/
            for (l = 0; l < factorial + factorial_inside; l++) {
              temp_nobj = 0;
              temp_outside_first_quadrant = 0;
              for (m = 0; m < nobj; m++) {
                if (fabs(adaptive_refpoints[m][temp_j] - ref_points[m][l]) <
                    1e-15) {
                  temp_nobj++;
                }
            ...
            for (m = 0; m < temp_j; m++) {
              ...
              temp_adaptive_refpoint_repeated = 0;
              for (n = 0; n < nobj; n++) {
                if (fabs(adaptive_refpoints[n][temp_j] -
                         adaptive_refpoints[n][m]) < 1e-15)
                  temp_nobj_adaptive++;
              }
              if (temp_nobj_adaptive == nobj) {
                temp_adaptive_refpoint_repeated = 1;
...
void add_adaptive_refpoints_to_ref_points() {
  ...
  /*Be sure there is not repeated the new adaptive reference point, before add
   * it*/
  for (j = factorial + factorial_inside;
       j < factorial + factorial_inside + adaptive_refpoint_number; j++) {
    for (k = 0; k < nobj; k++) {
      ref_points[k][j] = 0.0;
    }
  }
  for (i = 0; i < adaptive_refpoint_number; i++) {
    for (k = 0; k < nobj; k++) {
      ref_points[k][factorial + factorial_inside + i] =
          adaptive_refpoints[k][i];
    }
  }
  return;
}
int delete_adaptive_refpoints(int archieve_size, int front_size,
                              population *selection_pop, population *new_pop,
                              int generation) {
  /*If there are not adaptive reference points added, then go directly to the
   * next generation*/
  if (adaptive_refpoint_number == 0) return 0;
  int i, j, k, l;
  ...
  /*If there are adaptive reference points, then associate them to the
   * population*/
  for (l = 0; l < factorial + factorial_inside + adaptive_refpoint_number;
       l++) {
    rho_St[l] = 0;
    rho_Fl[l] = 0;
    rho[l] = 0;
  ...
  find_extreme_points(
      new_pop, popsize); /*only consider the individuals in the first front*/
  ...
  for (l = 0; l < popsize; l++) {
    associate(&new_pop->ind[l], l, 10 * popsize, 0,
              factorial + factorial_inside + adaptive_refpoint_number);
  }
  /*printf("Printing rho after adding adaptive refpoints and associate all
   * refpoints with the population\n");*/
...
void check_adaptive_refpoints_inclusion_number(int generation) {
  ...
  for (i = factorial + factorial_inside;
       i < factorial + factorial_inside + adaptive_refpoint_number; i++) {
    if (rho_St[i] > 0) {
      for (j = 0; j < elegible_adaptive_ref_points_to_be_fixed_number; j++) {
        temp_nobj = 0;
        for (k = 0; k < nobj; k++) {
          if (fabs(adaptive_ref_points_settled[k][j] - ref_points[k][i]) <
              1e-15) {
            temp_nobj++;
          }
        }
      ...
      if (temp_nobj == nobj) {
        /*printf("Settled adaptive reference point is already included, increase
         * the ocurrence number\n");*/
        adaptive_ref_points_settled_number[temp_j] =
            adaptive_ref_points_settled_number[temp_j] + 1;
        /*printf("elegible_adaptive_ref_points_to_be_fixed_number
         * %d\n",elegible_adaptive_ref_points_to_be_fixed_number);*/
        if (adaptive_ref_points_settled_number[temp_j] >= 2 &&
            (generation - last_generation_associated_rho_St[temp_j]) >= 10) {
          /*printf("New reference point added to Initial Das and Dennis
           * reference points\n");*/
          for (k = 0; k < nobj; k++) {
            ref_points[k][factorial + factorial_inside] =
                adaptive_ref_points_settled[k][temp_j];
            adaptive_ref_points_settled[k][temp_j] = adaptive_ref_points_settled
                [k][elegible_adaptive_ref_points_to_be_fixed_number - 1];
          }
          adaptive_ref_points_settled_number[temp_j] =
              adaptive_ref_points_settled_number
                  [elegible_adaptive_ref_points_to_be_fixed_number - 1];
          last_rho_St[temp_j] =
              rho_St[elegible_adaptive_ref_points_to_be_fixed_number - 1];
          adaptive_ref_points_settled_number
              [elegible_adaptive_ref_points_to_be_fixed_number - 1] = 0;
          elegible_adaptive_ref_points_to_be_fixed_number--;
          factorial++;
        ...
        last_rho_St[elegible_adaptive_ref_points_to_be_fixed_number] =
            rho_St[i];
        /*printf("New settled adaptive reference point is included, the
         * ocurrence number is \t");*/
        for (k = 0; k < nobj; k++) {
          adaptive_ref_points_settled
              [k][elegible_adaptive_ref_points_to_be_fixed_number] =
                  ref_points[k][i];
        }
        adaptive_ref_points_settled_number
            [elegible_adaptive_ref_points_to_be_fixed_number] =
        ...
        last_generation_associated_rho_St
            [elegible_adaptive_ref_points_to_be_fixed_number] = generation;
...
/*This function sorts the original reference points by rho_St, but just the
 * index, not the physical location
 */
void sort_all_refpoints_by_rho_index() {
  ...
  for (i = 0; i < factorial - adaptive_ref_points_inserted + factorial_inside;
       i++) {
  ...
  for (i = 0; i < factorial - adaptive_ref_points_inserted + factorial_inside;
       i++) {
    for (j = 0; j < factorial - adaptive_ref_points_inserted +
                        factorial_inside - i - 1;
         j++) {
  ...
  /*printf("\nSorting original Das and Dennis reference points by rho
  index\n________________________________________________\n"); printf("rho_st is
  Sorted from highest to lowest, using sort_all_refpoint_index[i]\n");*/
  return;
}
/*This function finds the index of the useless refpoints, the usefull and
  useless numbers It considers whether rho_St nor rho_fl are equal to zero*/
...
void store_useless_refpoints(int adaptive_ref_point_number,
                             int useless_ref_point_number) {
  int i, k;
  /*printf("Storing useless refpoints temporally.\nThose vectors are restored to
   * the next generation:\n");*/
...
void load_useless_refpoints(int adaptive_ref_point_number,
                            int useless_ref_point_number) {
  int i, k;
  for (i = 0; i < useless_ref_point_number; i++) {
    for (k = 0; k < nobj; k++) {
